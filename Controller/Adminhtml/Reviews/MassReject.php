...
class MassReject extends \Magento\Backend\App\Action implements HttpPostActionInterface
{
    ...
    public function execute()
    {
        ...
        $dealerReviewsRejected = 0;
        foreach ($collection as $dealerReview) {
            try {
                $dealerReview->setIsConfirmed(DealerReviewsInterface::STATUS_REJECTED);
                $this->dealerReviewsRepository->save($dealerReview);
                $dealerReviewsRejected++;
        ...
        if ($dealerReviewsRejected) {
            $this->messageManager->addSuccessMessage(__('A total of %1 record(s) have been rejected.', $dealerReviewsRejected));
        }
